...
export async function createPais(req: Request, res: Response) {
    ...
    let pais = new Pais(req.body.code, req.body.name);
    try {
        console.time(`Inserted country with name ${pais.name}`);
        await db().
        query('INSERT INTO pais VALUES (default, $2)',
            [pais.name]);
        console.timeEnd(`Inserted person with id ${pais.name}`);
...
export async function updatePais(req: Request, res: Response) {
    ...
    let pais = new Pais(parseInt(paisCode), req.body.name);
    try {
