...
export async function createMunicipio(req: Request, res: Response) {
    ...
    let municipio = new Municipio(req.body.code, req.body.name, req.body.codeEstado);
    try {
        console.time(`Inserted municipality with name ${municipio.name}`);
        await db().
        query('INSERT INTO municipio VALUES (default, $2, $3)',
            [municipio.name, municipio.codeEstado]);
        console.timeEnd(`Inserted municipio with id ${municipio.name}`);
...
export async function updateMunicipio(req: Request, res: Response) {
    ...
    let municipio = new Municipio(parseInt(municipioCode), req.body.name, req.body.codeEstado);
    try {
        await db().query(`UPDATE municipio
                      SET name=$2, codeEstado=$3
                      WHERE code = $1`,
