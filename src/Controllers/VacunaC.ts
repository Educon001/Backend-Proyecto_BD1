...
export async function createVacuna(req: Request, res: Response) {
    ...
    let vacuna = new Vacuna(req.body.name, parseInt(req.body.lote), parseInt(req.body.cantDosis), req.body.type, req.body.laboratory, parseInt(req.body.code_Pais));
    try {
        console.time(`Inserted vacuna with name ${vacuna.name}`);
        await db().
        query('INSERT INTO vacuna VALUES (default, $1, $2, $3, $4, $5, $6)',
            [vacuna.name, vacuna.lote, vacuna.cantDosis, vacuna.type, vacuna.laboratory, vacuna.code_Pais]);
...
export async function updateVacuna(req: Request, res: Response) {
    ...
    let vacuna = new Vacuna(req.body.name, parseInt(req.body.lote), parseInt(req.body.cantDosis), req.body.type, req.body.laboratory, parseInt(req.body.codePais), parseInt(vacunaCode));
    try {
        await db().query(`UPDATE vacuna
                      SET name=$2, lote=$3, cantDosis=$4, type=$5, laboratory=$6, code_pais=$7
                      WHERE code=$1
                      `,
            [vacuna.code, vacuna.name, vacuna.lote, vacuna.cantDosis, vacuna.type, vacuna.laboratory, vacuna.code_Pais]);
        return res.json(vacuna);
    } catch (e) {
        console.error(e);
        return res.status(400).json({message: 'Bad Request'});
